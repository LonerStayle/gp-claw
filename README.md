...
자체 호스팅 LLM(Qwen2.5-14B-Instruct) + RunPod Serverless GPU.
...
4. **Deploy** 버튼 클릭
5. **MODEL NAME**에 `Qwen/Qwen2.5-14B-Instruct` 입력
6. 서버리스 생성 완료
...
2. **GPU Configuration**에서 사용할 GPU 선택
   - `Qwen/Qwen2.5-14B-Instruct`는 **48GB** VRAM 필요 (A40, A6000 등)
...
| 변수명 | 값 | 설명 |
|--------|-----|------|
| `MAX_MODEL_LEN` | `16384` | 최대 컨텍스트 길이 (모델마다 다름 Qwan2.5 기준입니다.) |
...
> - 이전 모델에서 사용하던 값(예: 31021)을 그대로 두면 CUDA OOM 에러가 납니다.
