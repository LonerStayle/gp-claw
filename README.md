...
회사 내부 AI 사무 비서. 파일 읽기/쓰기/검색/삭제, 위험 작업은 사람 승인 필수.
자체 호스팅 LLM + RunPod Serverless GPU.
...
4. **Deploy** 버튼 클릭
5. **MODEL NAME**에 사용할 모델명 입력 (모르겠으면 `skt/A.X-4.0-Light` 사용)
6. 서버리스 생성 완료
...
2. **GPU Configuration**에서 사용할 GPU 선택
   - 모델마다 필요한 VRAM이 다릅니다
   - `skt/A.X-4.0-Light`의 경우 **48GB** 이상이면 빠릅니다
...
| `ENABLE_AUTO_TOOL_CHOICE` | `true` | 자동 도구 선택 활성화 |
| `NUM_GPU_BLOCKS_OVERRIDE` | `31021` | GPU 블록 수 오버라이드 |
| `VLLM_CLI_ARGS` | `--enable-auto-tool-choice --tool-call-parser hermes` | vLLM CLI 추가 인자 |
...
2. 채팅 입력창에 자연어로 요청 (예: "파일 목록 보여줘", "report.txt 읽어줘")
3. AI가 응답합니다
...
파일 쓰기/삭제/이동 같은 위험 작업은 승인 카드가 표시됩니다.
**승인** 또는 **거부**를 선택하세요.
