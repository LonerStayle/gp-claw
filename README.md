...
자체 호스팅 LLM(Qwen3-8B) + RunPod Serverless GPU.
...
4. **Deploy** 버튼 클릭
5. **MODEL NAME**에 `Qwen/Qwen3-8B` 입력
6. 서버리스 생성 완료
...
2. **GPU Configuration**에서 사용할 GPU 선택
   - `Qwen/Qwen3-8B`는 **24GB+** VRAM 필요 (A30, L4, A40, A6000 등)
   - 모델 ~16GB + KV cache 여유 필요
...
| 변수명 | 값 | 설명 |
|--------|-----|------|
| `MAX_MODEL_LEN` | `32768` | 최대 컨텍스트 길이 (Qwen3-8B 기본 32K) |
...
> - RunPod vLLM은 모델 이름을 소문자로 등록합니다 (예: `qwen/qwen3-8b`)
> - `.env`의 `VLLM_MODEL_NAME`이 RunPod 로그의 `served_model_name`과 정확히 일치해야 합니다
> - 대소문자 불일치 시 500 에러가 발생합니다
